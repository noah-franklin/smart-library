<?php
    ...
    $query = "SELECT * FROM shelflocations_f20 WHERE Map = 1 ORDER BY ShelfNo ASC";
    $query2 = "SELECT * FROM permstructlocations_f20 WHERE Map = 1 ORDER BY StructID ASC";
    ...
    while($row = mysqli_fetch_array($result)){
        ...
        if(!in_array("MISSING", $list[$i])) {
            ...
            $string = "<svg width=&ldquo;5&rdquo; height=&ldquo;20&rdquo;><div onclick=getBookCase(" .$list[$i]['Bookcase'] . ")><rect class=\"" . $list[$i]['Class'] . "\" id =\"" . $list[$i]['ShelfNo'] . "\" width=\"" . $list[$i]['Height'] . "\" height=\"" . $list[$i]['Width'] . "\" x=\"" . $list[$i]['X'] . "\" y=\"" . $list[$i]['Y'] . "\" style=\"cursor:pointer;\"/></a></svg>";
            echo($string);
        }
        $i++;
    }
